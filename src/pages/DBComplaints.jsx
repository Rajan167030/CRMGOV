  ...
  if (selected) return (
    <div style={{ padding: p }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 300px", gap: 18 }}>
        <div>
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 13, padding: isMobile ? "18px" : "26px", marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
              <div>
                <span style={{ color: T.cyan, fontFamily: "monospace", fontSize: 13, fontWeight: 700 }}>{selected.id}</span>
                <h2 style={{ color: T.text, fontSize: isMobile ? 17 : 20, fontWeight: 900, margin: "6px 0 4px", fontFamily: "'Syne',sans-serif" }}>{selected.dept} Complaint</h2>
  ...
  return (
    <div style={{ padding: p }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search complaints…" style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 8, padding: "8px 14px", color: T.text, fontSize: 13, outline: "none", width: isMobile ? "100%" : 210 }} />
      </div>
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 13, overflow: "hidden" }}>
        ...
        {isMobile ? (
          <>
            ...
            {filtered.map((c, i) => (
              <div key={c.id} onClick={() => setSelected(c)} style={{ padding: "14px 16px", cursor: "pointer", borderBottom: i < filtered.length - 1 ? `1px solid ${T.border}` : "none" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ color: T.cyan, fontFamily: "monospace", fontSize: 12, fontWeight: 700 }}>{c.id}</span>
                  <Badge status={c.status} />
                </div>
                <div style={{ color: T.text, fontSize: 14, fontWeight: 600, marginBottom: 4 }}>{c.citizen}</div>
