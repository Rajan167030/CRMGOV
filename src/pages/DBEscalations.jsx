    ...
    <div style={{ padding: p }}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,1fr)", gap: 12, marginBottom: 20 }}>
        {[["Active Escalations", "3", T.red], ["Avg Overdue", "2.1 days", T.amber], ["Supervisors Assigned", "3", T.green]].map(([k, v, col]) => (
          <div key={k} style={{ background: T.card, border: `1px solid ${col}33`, borderRadius: 11, padding: isMobile ? "14px 16px" : "18px 20px" }}>
      ...
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 13, overflow: "hidden" }}>
        ...
        {esc.map((c, i) => (
          <div key={c.id} style={{ padding: isMobile ? "14px 16px" : "16px 20px", borderBottom: i < esc.length - 1 ? `1px solid ${T.border}` : "none" }}>
