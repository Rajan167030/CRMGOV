    ...
    <div style={{ padding: p }}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 14 : 18, marginBottom: isMobile ? 14 : 18 }}>
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 13, padding: isMobile ? "16px" : "20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: isMobile ? 4 : 7, height: 150 }}>
            {MONTHLY.map((b, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  ...
                  <span style={{ color: T.sub, fontSize: isMobile ? 7 : 9 }}>{b.m}</span>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 13, padding: isMobile ? "16px" : "20px", marginBottom: isMobile ? 14 : 18 }}>
        ...
        <div style={{ height: isMobile ? 180 : 220, background: `linear-gradient(135deg,${T.bg},#0a1525)`, borderRadius: 9, border: `1px solid ${T.border}`, position: "relative", overflow: "hidden" }}>
          ...
          {[{ x: 28, y: 38, r: isMobile ? 50 : 70, col: T.red, label: "Delhi NCR", n: 312 }, { x: 58, y: 62, r: isMobile ? 38 : 52, col: T.amber, label: "Bangalore", n: 187 }, { x: 72, y: 28, r: isMobile ? 30 : 40, col: T.cyan, label: "Hyderabad", n: 143 }, { x: 18, y: 68, r: isMobile ? 26 : 34, col: T.purple, label: "Mumbai", n: 112 }, { x: 84, y: 58, r: isMobile ? 20 : 26, col: T.green, label: "Chandigarh", n: 76 }].map((b, i) => (
            <div key={i} style={{ position: "absolute", left: `${b.x}%`, top: `${b.y}%`, transform: "translate(-50%,-50%)" }}>
              ...
              <div style={{ color: b.col, fontSize: isMobile ? 7 : 9, fontWeight: 700, textAlign: "center", marginTop: 3, textShadow: "0 1px 4px #000" }}>{b.label}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: 12 }}>
        {[["Citizen Satisfaction", "78%", "Avg 3.9/5", T.green], ["First Response", "4.2h", "Target < 6h", T.cyan], ["Repeat Complaints", "12%", "Down from 18%", T.amber], ["Digital Adoption", "68%", "Web + Mobile", T.purple]].map(([l, v, s, col]) => (
          <div key={l} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 11, padding: isMobile ? "14px" : "18px" }}>
            <div style={{ color: col, fontSize: isMobile ? 22 : 28, fontWeight: 900, fontFamily: "'Syne',sans-serif" }}>{v}</div>
            <div style={{ color: T.text, fontSize: isMobile ? 11 : 13, fontWeight: 700, marginTop: 6 }}>{l}</div>
